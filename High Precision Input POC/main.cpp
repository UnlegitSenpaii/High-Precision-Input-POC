...
DWORD WINAPI TickHandler() 
{
	...
	std::vector<KeyboardEvent> KeyboardEventsCopy;
	std::chrono::high_resolution_clock::time_point now = std::chrono::high_resolution_clock::now();
	...
	for (;;) 
	{
		...
		// Process MouseEvents
		{
			...
			std::lock_guard<std::mutex> mouseGuard(mouseEventsMutex);
			MouseEventsCopy = MouseEvents;
			MouseEvents.clear();
		}

		// Process KeyboardEvents
		{
			KeyboardEventsCopy.clear();
			// Lock the mutex before accessing shared data
			std::lock_guard<std::mutex> keyboardGuard(keyboardEventsMutex);
			KeyboardEventsCopy = KeyboardEvents;
		...
		if (KeyboardEventsCopy.size() > 0)
			output << "Received " << KeyboardEventsCopy.size() << " keyboard events since last tick." << std::endl;
...
LRESULT CALLBACK RawInputProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	if (uMsg == WM_INPUT) {
		...
		if (raw.header.dwType == RIM_TYPEMOUSE) {
			...
			KeyboardEvent ke;
			ke.key = raw.data.keyboard.VKey;
			ke.timestamp = std::chrono::high_resolution_clock::now();

			ke.keyState = 0;
			if (raw.data.keyboard.Message == WM_KEYDOWN) ke.keyState = 1;
			if (raw.data.keyboard.Message == WM_KEYUP) ke.keyState = 2;

			{
				// Lock the mutex before accessing shared data
				std::lock_guard<std::mutex> guard(keyboardEventsMutex);
				// Add KeyboardEvent to the vector
				KeyboardEvents.push_back(ke);
				// Close mutex by ending brackets
			}
		}
	}
